...
def main() -> None:
    ...
    source_path = data["source_path"].iloc[0]["source_url"]

    render_header(source_path, matched_df)
    filtered_df = render_filters(matched_df)

    kpi = build_kpis(filtered_df)
    render_kpis(kpi)
    render_charts(filtered_df)
    render_tables(filtered_df)


def render_header(source_path: str, matched_df: pd.DataFrame) -> None:
...
def build_kpis(df: pd.DataFrame) -> dict[str, float]:
    ...
    return {
        ...
        "페이지수": float(df["page_count"].sum()),
        "결제수": float(df["payment_count"].sum()),
        "결제금액": float(payment_amount),
        "ROAS": float(roas),
    }


def render_kpis(kpi: dict[str, float]) -> None:
    labels = list(kpi.keys())
    columns = st.columns(4)
    for idx, label in enumerate(labels):
        with columns[idx % 4]:
            value = kpi[label]
            if label in {"총 작업 수", "고객수", "유입수", "페이지수", "결제수"}:
