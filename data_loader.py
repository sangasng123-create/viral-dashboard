...
def load_data() -> dict[str, pd.DataFrame]:
    ...
    """
    raw_data = load_google_public_sheets_data()
    ...
    return {
        ...
        "source_path": pd.DataFrame([{"source_url": SPREADSHEET_URL}]),
    }
...
def load_google_public_sheets_data() -> dict[str, pd.DataFrame]:
...
def match_paid_with_performance(
    ...
    for _, row in paid_df.iterrows():
        ...
        if matched is None:
            reverse_match = find_reverse_match(row, perf_candidates)
...
def collapse_cumulative_performance_rows(
    ...
    for _, group in working.groupby(key_columns, dropna=False, sort=False):
        ...
        # 누적형 데이터가 아래로 계속 쌓이는 구조를 가정하고,
        # 같은 키/같은 최신 수집일 안에서는 가장 큰 숫자를 최종값으로 사용한다.
        for metric in PERFORMANCE_METRICS:
            row_data[metric] = latest_group[metric].max()
...
def find_reverse_match(row: pd.Series, perf_candidates: list[dict]) -> dict | None:
    ...
    relevant = [
        ...
        if candidate.get("nt_source") in source_candidates and candidate.get("nt_detail") == worker
    ]
    if not relevant:
        return None
